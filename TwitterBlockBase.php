...
class TwitterBlockBase extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  /**
   * Pull Tweets from Twitter API. Add array $proxy in parameters
   *
   * @return array
   */
  public function getTweets(string $url, string $parameters, array $proxy ) {
    $tweets = [];

    //Get proxy host:port
    $this->proxy = !empty($proxy) ? $proxy['proxy']['https'] : '';

    try {
      if (empty($this->proxy)) {
        $result = $this->twitter->setGetfield($parameters)->buildOauth($url, 'GET')->performRequest();
      } else {
        $result = $this->twitter->setGetfield($parameters)->buildOauth($url, 'GET')->performRequest(TRUE, [CURLOPT_PROXY => $this->proxy]);
      }
